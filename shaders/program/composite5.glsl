...
void main(){
	...
	#ifdef ROLLOVER
    	float texOffset = worldTime / 200.0 + sin(worldTime / 250.0) + 0.01 * sin(worldTime * 0.5) + mod(newTexCoord.x, 1/viewWidth);
    	texOffset = mod(texOffset, 1);
    	texOffset = mod((1 - 2 * texOffset) / abs(2 - 4 * texOffset + 0.0001) * pow(abs(1 - 2 * texOffset), 80), 1);
    	texOffset *= 1.03;
    	texOffset -= clamp(0.1 * (1 - pow(1 - 2 * texOffset, 2)) * newTexCoord.x * newTexCoord.x - 0.001, 0, 1);
		newTexCoord = vec2(newTexCoord.x, mod(texOffset + newTexCoord.y, 1.03));
    #endif
    
	#if CHROMATIC_ABERRATION == 0
	vec3 color = texture2D(colortex0, newTexCoord).rgb;
	#else
    	vec2 dTexCoord = texCoord - 2 * texCoord + vec2(0.5);
    	vec3 color = vec3(texture2D(colortex0, newTexCoord + 0.0002 * CHROMATIC_ABERRATION * dTexCoord).r, texture2D(colortex0, newTexCoord + 0.0001 * CHROMATIC_ABERRATION * dTexCoord).g, texture2D(colortex0, newTexCoord).b);
	#endif
	
	if(newTexCoord.y > 1.0) color = vec3(0);
	...
	vec3 filmGrain = TextureSample3R(noisetex, filmGrainCoord);
