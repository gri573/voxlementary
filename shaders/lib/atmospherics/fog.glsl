...
vec3 Fog1(vec3 color, float lWorldPos, float lViewPos, vec3 nViewPos, vec3 extra, float NdotU) {
    ...
    #ifdef NETHER // extra = nether smoke (if enabled)
		...
		#if END_SKY == 2 && !defined COMPATIBILITY_MODE
			float dither = Bayer64(gl_FragCoord.xy);
			vec4 nebula = DrawEndCloud(extra.xyz, dither, endCol);
...
vec3 Fog2(vec3 color, float lViewPos, vec3 worldPos) {
		...
		float timeBrightness2 = 1.0 - (1.0 - timeBrightness) * (1.0 - timeBrightness);
		vec3 fogColor2 = mix(lightCol*0.5, pow(skyCol, vec3(0.7))*1.15, timeBrightness2);
		...
		if (rainStrengthS > 0.0) {
			...
			fogColor2 = mix(fogColor2, FOG2_RAIN_BRIGHTNESS_M * skyCol * (0.01 + 0.05 * sunVisibility8 + 0.1 * timeBrightness2), rainStrengthS2);
		}
...
vec3 startFog(vec3 color, vec3 nViewPos, float lViewPos, vec3 worldPos, vec3 extra, float NdotU) {
	...
	#if (defined FOG1 || defined FOG2) && defined OVERWORLD
		if (eyeAltitude > 300.0) {
			float veryHighAltitude = clamp(lViewPos / 45.0, 0.0, 1.0) * (eyeAltitude - 300.0) * 0.005;
			color.rgb = mix(color.rgb, skyFogColor, min(veryHighAltitude * veryHighAltitude, 1.0));
		}
	#endif
	*/
