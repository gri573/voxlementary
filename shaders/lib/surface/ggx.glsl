...
float GGX(vec3 normal, vec3 viewPos, vec3 lightVec, float smoothness, float f0, float sunSize) {
    ...
    if (sunVisibility == 0.0) specular *= float(moonPhase == 0) * 0.35 + 0.65 - float(moonPhase == 4) * 0.60;

    return specular * (1.0 - isEyeInWater*0.75);
}

float stylizedGGX(vec3 normal, vec3 oldNormal, vec3 nViewPos, vec3 lightVec, float smoothness, float f0, float sunSize) {
    float roughness = 1.0 - smoothness;
    roughness = 0.5;
    ...
    float dotLH = clamp(dot(halfVec, lightVec), 0.0, 1.0);
    float dotNewL = clamp(dot(oldNormal,  lightVec), 0.0, 1.0);
    float dotNmOL = clamp(dot(normal - oldNormal,  lightVec), 0.0, 1.0);
    
    float dotNH = GetNoHSquared(sunSize, dotNewL, dot(oldNormal, -nViewPos), dot(-nViewPos, lightVec));
    ...
    float timeBrightnessM = timeBrightness * timeBrightness + moonBrightness * moonBrightness;
    timeBrightnessM *= timeBrightnessM;
    timeBrightnessM *= timeBrightnessM;
    timeBrightnessM *= timeBrightnessM;

    if (sunVisibility == 0.0) specular *= 0.15;
    specular *= 0.5 + 0.75 * dotNmOL;
...
vec3 GetSpecularHighlight(float smoothness, float metalness, float f0, vec3 specularColor,
                          vec3 rawAlbedo, float shadow, vec3 normal, vec3 viewPos, float materialFormat){
    #ifndef SHADOWS
        return vec3(0.0);
    #endif
